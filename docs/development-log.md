...
2. **שיפור תפריט הפעולות**
   ...
   - הוספת אישור מחיקה לני מחיקת משימה
...
7. **אינטגרציה**:
   ...
   - מימוש בנתונים אמיתיים מהמשימות
...
### הערות טכניות
- נעשה שימוש ב-Supabase Auth לניהול משתמשים
...
## 26.12.2024 - הועלתא גירסא 13 שמטפלת בבאג איפוס סיסמא, ועדכנה תמונה ושם בטאב האפליקציה.


## 26.12.2024 - עדכון אזור פרטי משתמש בדף הגדרות מהדטבייס

### שינויים עיקריים
1. **הוספת תמיכה בתמונות פרופיל**
   - הקמת Bucket חדש `avatars` ב-Supabase לאחסון תמונות
   - הגדרת מדיניות אבטחה והרשאות לניהול תמונות
   - תמיכה בהעלאת תמונות בגרירה (drag & drop)
   - ולידציה של גודל וסוג הקובץ
   - אפשרות להסרת תמונת פרופיל

2. **שיפור ניהול פרטי משתמש**
   - הרחבת טבלת `profiles` עם עמודות `name` ו-`avatar_url`
   - שיפור הטיפול בעדכון פרטי משתמש
   - תיקון באג שגרם לאיבוד נתונים בעת עדכון במקביל של שם ותמונה
   - הוספת ולידציה לשדה השם

3. **שיפורי UX**
   - הוספת אינדיקציה ויזואלית בזמן העלאת תמונה
   - שיפור המשוב למשתמש במקרה של שגיאות
   - תצוגת ברירת מחדל של האות הראשונה מהשם/אימייל כשאין תמונה

### שיפורי אבטחה
- הגדרת Row Level Security (RLS) לטבלת `profiles`
- הגדרת מדיניות אבטחה לקבצים ב-Storage
- וידוא שמשתמשים יכולים לערוך רק את הפרופיל שלהם

### הערות טכניות
- נעשה שימוש ב-Supabase Storage לניהול קבצים
- הוגדרו מדיניויות אבטחה ברמת ה-SQL
- שופר ניהול ה-state בקומפוננטת הפרופיל

### משימות להמשך
1. [ ] הוספת חיויי פרויפיל המשתמש והתנתקות למעלה בדף
...
1. **שיפור הגדרות נגישות**
   - הסרת הגדרות מיותרות (ניגודיות גבוהה והפחתת אנימציות)
   - שיפור מנגנון שינוי גודל טקסט:
     - קטן: 14px (כותרות: 20px, 18px, 16px)
     - בינוני: 18px (כותרות: 28px, 24px, 20px)
     - גדול: 22px (כותרות: 32px, 28px, 24px)
   - שינוי המימוש כך שישפיע רק על טקסט ולא על כל האפליקציה
   - שימוש ב-data attributes במקום classes לשליטה בגודל הטקסט
   - שיפור התצוגה המקדימה בהגדרות

## 02.01.2025 - הגדרת סביבות עבודה והעלאה לVercel
גירסא - 15
### שינויים עיקריים
